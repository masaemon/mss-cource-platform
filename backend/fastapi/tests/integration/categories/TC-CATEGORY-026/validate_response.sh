#!/bin/bash
# TC-CATEGORY-026: Validate unique slugs (database constraint check)
...
if [ "$UNIQUE_SLUGS" != "true" ]; then
    echo "✗ Duplicate slugs found (constraint violation)"
    exit 1
fi

echo "✓ HTTP status 200 confirmed"
echo "✓ All slugs are unique (constraint satisfied)"
exit 0
